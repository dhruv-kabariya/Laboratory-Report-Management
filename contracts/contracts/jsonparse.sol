...
library JsmnSolLib {
    ...
    function parse(string json, uint numberElements) internal returns (uint, Token[], uint) {
        ...
        for (; parser.pos<s.length; parser.pos++) {
            ...
            if (c == 0x7b || c == 0x5b) {
                ...
                bool success;
                Token memory tokenn;
                (success, tokenn)= allocateToken(parser, tokens);
                if (!success) {
                ...
                tokenn.jsmnType = (c == 0x7b ? JsmnType.OBJECT : JsmnType.ARRAY);
                tokenn.start = parser.pos;
                tokenn.startSet = true;
    ...
    function parseBool(string _a) public returns (bool) {
